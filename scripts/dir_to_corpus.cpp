...
enum class OutputSetting : char {
  NUMBER, TEXT
} output_setting;
struct Term {
  int word;
  int count;
  bool seen_in_this_document;
};
...
bool acceptableLength(const std::string& word) {
...
void initializeStoplist(const std::string& doc_dir, int stopword_count, int minimum_frequency) {
  std::unordered_map<std::string, int> words_and_freqs;
  // The files are not sorted as they are in the directory.
  for (auto& file_entry : fs::directory_iterator{ doc_dir }) {
    std::ifstream file{ file_entry.path().string() };
    std::string raw_word;
    while (file >> raw_word) {
  ...
  std::move(std::begin(words_and_freqs), std::end(words_and_freqs), std::back_inserter(freq_vec));
  std::sort(std::begin(freq_vec), std::end(freq_vec), [](const auto& p1, const auto& p2) {
  ...
  for (auto from_last = std::rbegin(freq_vec), from_first = std::rend(freq_vec);
      from_last != from_first and from_last->second < minimum_frequency; ++from_last) {
    stoplist.emplace(std::move(from_last->first));
  }
}

int readSingleFile(const std::string& filename, std::ofstream& vocabulary) {
...
void writeFileTerms(std::ofstream& data_file, int current_count) {
  ...
  for (auto& lexicon_data : lexicon) {
    ...
    if (output_setting == OutputSetting::TEXT) {
      ...
      data_file << lexicon_data.second.word << ':' << lexicon_data.second.count << ' ';
    }
...
// Compile with '-std=c++1z <file> -lstdc++fs'.
int main(int argc, const char* argv[]) {
  if (argc < 3) {
    std::cout << "usage: ./dir_to_corpus <document directory> { text | number } "
              "[<minimum frequency (exclusive)>] [<num stopwords>] "
              "[<corpus output>] [<vocabulary output>]\n";
    std::exit(EXIT_FAILURE);
  }
  if (not fs::exists(argv[1])) {
  ...
  for (auto& file_entry : fs::directory_iterator{ argv[1] }) {
    int current_count = readSingleFile(file_entry.path().string(), vocabulary_file);
